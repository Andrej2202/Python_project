...
class Airport:
    ...
    def send_aircraft(self, target_airport, number, distance):
        ...
        if type(aircraft_sent) == Plane and target_airport.line_lengths < aircraft_sent.min_line_length:
            ...
            return
        if type(aircraft_sent) == Helicopter and aircraft_sent.is_miliraty == True and target_airport.max_aircrafts*0.5 < len(target_airport.planes+target_airport.helicopters):
            print('Aircraft maximum capacity reached')
            return
        ...
        if type(target_airport) == Helicopter:
